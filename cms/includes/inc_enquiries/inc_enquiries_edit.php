...
function edit_item() {
    ...
    $row = fetch_row("SELECT LPAD(`id`, 4, 0) AS ind, TRIM(CONCAT(`first_name`, ' ', `last_name`)) AS name, `email_address`, `contact_number`, `comments`, DATE_FORMAT(`date_of_enquiry`, '%e %M %Y @ %h:%i %p') AS date_enquired
            FROM `enquiry`
            WHERE `id` = '$id'
            LIMIT 1");
