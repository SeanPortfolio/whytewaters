...
function view_trash()
{
    ...
    function generate_item_list($parent_id = 0)
    {
        ...
        $sql = "SELECT cs.`id`, pmd.`name`, pmd.`id` AS page_meta_id, pmd.`status`, pmd.`rank`,
        DATE_FORMAT(pmd.`date_deleted`, '%d %M %Y %h:%i %p') AS deleted_date, c.`name` AS client_name
        FROM `case_study` cs
        ...
        if(count($rows) > 0)
        {
            ...
            foreach ($rows as $index => $row)
            {
                ...
                $label = ($client_name) ? str_truncate($client_name, 70) : $name;
