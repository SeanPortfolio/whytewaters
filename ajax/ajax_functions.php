...
switch($action)
{
	...
	break;
	case 'fetch-gallery':
		get_gallery_photos( sanitize_var($request_type['key'], FILTER_VALIDATE_INT) );
	break;
}
...
function get_gallery_photos( $gallery_id )
{
	...
	if( $gallery_id )
	{
		$data = fetch_all("SELECT `full_path` AS src, `thumb_path` AS msrc, `width` AS w, `height` AS h, `caption` AS title
			FROM `photo`
